...
from climatepy.figures import StyleMixin, DrawMixin, FIGURE_SIZE, FONT_NAME, FONT_SIZE, FORMAT
import numpy as np
import abc
import fiona
...
class BaseMap(StyleMixin, DrawMixin):
    ...
    def __init__(self, name, bounds, **kwargs):
        opts = kwargs or {}
        self.bounds = [0, 0, 0, 0]
        if len(bounds) > 1 and isinstance(bounds[0], tuple) and isinstance(bounds[1], tuple):
            self.bounds = []
    ...
    @abc.abstractmethod
    def add_contour(self, x, y, data, unit=False, lines=True, alt=False, extend="neither"):
        pass
...
class MPLMap(BaseMap):
    ...
    def add_shape(self, name, file_name, field_name, linewidth=0.2, color="k"):
        self.map.readshapefile(file_name, name, linewidth=linewidth, color=color)
        with fiona.open(file_name + "_pt.shp") as shp:
            for row in shp:
                ...
                if (x < self.bounds[0] or x > self.bounds[2]) or \
                    ...
                    continue
                self.axes.text(x, y, row["properties"][field_name].replace(" ", "\n"),
                               ha="center", va="center", rotation=0,
                               **MPLMap.style("shapetext"))

    def add_contour(self, x, y, data, unit=False, lines=True, alt=False, extend="neither"):
        # Options for extends are neither or both
        levels = np.linspace(*self.levels)
        cs = self.axes.contourf(x, y, data, levels,
                                cmap=self.color_map if not alt else self.alt_color_map,
                                extend=extend)
        cbar = self.figure.colorbar(cs, ticks=cs.levels, drawedges=True, shrink=0.78, pad=0.01)
        if isinstance(unit, basestring) or isinstance(unit, unicode):
            cbar.set_label(unit, position=(0, 1), rotation="horizontal", **MPLMap.style("colorbar"))
        if lines:
            cs2 = self.axes.contour(x, y, data, levels, cmap=self.color_line if not alt else self.alt_color_line,
                                    **MPLMap.style("colorline"))
            self.axes.clabel(cs2, cs.levels, inline=True, fmt='%.0f', lw=1, **MPLMap.style("colorline"))
            cbar.add_lines(cs2)
    ...
    def clear(self):
        ...
        self.map = bmap.Basemap(llcrnrlon=self.bounds[0], llcrnrlat=self.bounds[1],
                                urcrnrlon=self.bounds[2], urcrnrlat=self.bounds[3])
        self.map.drawparallels(np.arange(self.bounds[1], self.bounds[3], 1), labels=[1, 0, 0, 0], **MPLMap.style("axis"))
        self.map.drawmeridians(np.arange(self.bounds[0], self.bounds[2], 1), labels=[0, 0, 0, 1], **MPLMap.style("axis"))
